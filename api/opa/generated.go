...
type ServerInterface interface {
	...
	// (POST /v1/data)
	EvaluateDocument(w http.ResponseWriter, r *http.Request, params EvaluateDocumentParams)
}
...
func (siw *ServerInterfaceWrapper) EvaluateDocument(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	...
	if valueList, found := headers[http.CanonicalHeaderKey("X-Userinfo")]; found {
		...
		n := len(valueList)
		if n != 1 {
			siw.ErrorHandlerFunc(w, r, &TooManyValuesForParamError{ParamName: "X-Userinfo", Count: n})
			return
		}

		err = json.Unmarshal([]byte(valueList[0]), &XUserinfo)
		if err != nil {
			siw.ErrorHandlerFunc(w, r, &UnmarshalingParamError{ParamName: "X-Userinfo", Err: err})
			return
		}
	...
	handler := http.Handler(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		siw.Handler.EvaluateDocument(w, r, params)
	}))
...
func HandlerWithOptions(si ServerInterface, options StdHTTPServerOptions) http.Handler {
	...
	m.HandleFunc("POST "+options.BaseURL+"/v1/data", wrapper.EvaluateDocument)
...
// EvaluateDocument operation middleware
func (sh *strictHandler) EvaluateDocument(w http.ResponseWriter, r *http.Request, params EvaluateDocumentParams) {
	...
	if err != nil {
		...
		if err := validResponse.VisitEvaluateDocumentResponse(w); err != nil {
			sh.options.ResponseErrorHandlerFunc(w, r, err)
		}
